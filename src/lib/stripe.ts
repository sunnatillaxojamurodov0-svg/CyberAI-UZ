...
export async function handleWebhook(
  ...
  switch (event.type) {
    case "checkout.session.completed": {
      ...
      await db
        .prepare(
          `INSERT INTO subscriptions (id, user_id, stripe_customer_id, stripe_subscription_id, stripe_price_id, plan, status, current_period_start, current_period_end, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, 'active', ?, ?, ?, ?)
    ...
    case "invoice.payment_succeeded": {
      ...
      await db
        .prepare(
          `INSERT INTO payments (id, user_id, stripe_payment_intent_id, stripe_invoice_id, amount, currency, status, description, created_at)
    ...
    case "customer.subscription.updated": {
      ...
      await db
        .prepare(
          `UPDATE subscriptions SET status = ?, current_period_start = ?, current_period_end = ?, cancel_at_period_end = ?, updated_at = ? WHERE stripe_subscription_id = ?`,
        )
        .bind(
          status,
...
export async function getUserSubscription(
  userId: string,
): Promise<{ plan: Plan; status: string; currentPeriodEnd: number } | null> {
  const env = getEnv();
  const db = env.cyberai_db as D1Like;

  const sub = await db
    .prepare(
      "SELECT plan, status, current_period_end FROM subscriptions WHERE user_id = ? AND status != ? ORDER BY created_at DESC LIMIT 1",
    )

...
const TEMPLATES = {
  "ubuntu-web": {
    ...
    Cmd: [
      "/bin/bash",
      "-c",
      "apt-get update -qq && apt-get install -y -qq apache2 mysql-server openssh-server > /dev/null 2>&1 && mkdir -p /run/sshd && service apache2 start && service mysql start && /usr/sbin/sshd -D",
    ],
  ...
  "centos-mail": {
    ...
    Cmd: [
      "/bin/bash",
      "-c",
      "dnf install -y postfix dovecot openssh-server > /dev/null 2>&1 && mkdir -p /run/sshd && postfix start && dovecot && /usr/sbin/sshd -D",
    ],
  ...
  "debian-db": {
    ...
    Cmd: [
      "/bin/bash",
      "-c",
      "apt-get update -qq && apt-get install -y -qq postgresql redis-server openssh-server > /dev/null 2>&1 && mkdir -p /run/sshd && pg_ctlcluster 15 main start && redis-server --daemonize yes && /usr/sbin/sshd -D",
    ],
  ...
  "alpine-container": {
    ...
    Cmd: [
      "/bin/sh",
      "-c",
      "apk add --no-cache openssh docker-cli > /dev/null 2>&1 && ssh-keygen -A && /usr/sbin/sshd -D",
    ],
    ExposedPorts: { "22/tcp": {} },
    HostConfig: {
  ...
  "kali-pentest": {
    ...
    Cmd: [
      "/bin/bash",
      "-c",
      "apt-get update -qq && DEBIAN_FRONTEND=noninteractive apt-get install -y -qq kali-linux-headless openssh-server > /dev/null 2>&1 && mkdir -p /run/sshd && /usr/sbin/sshd -D",
    ],
    ExposedPorts: { "22/tcp": {} },
    HostConfig: {
...
async function syncContainers() {
  try {
    ...
    const cyberaiContainers = containers.filter(
      (c) => c.Labels && c.Labels["cyberai.managed"] === "true",
    );
...
async function cleanupExpiredContainers() {
  ...
  const runningCount = Array.from(activeContainers.values()).filter(
    (c) => c.status === "running",
  ).length;
...
app.get("/api/containers", authenticate, async (req, res) => {
  try {
    ...
    res.json({
      ok: true,
      data: enriched,
      meta: { max: MAX_CONTAINERS, ttl_min: CONTAINER_TTL_MS / 60000 },
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        state,
        health,
        remaining_min: Math.max(
          0,
          Math.round((data.ttl_ms - (Date.now() - data.created_at)) / 60000),
        ),
      },
    });
  } catch (err) {
...
app.get("/api/health", async (req, res) => {
  try {
    await docker.ping();
    const running = Array.from(activeContainers.values()).filter(
      (c) => c.status === "running",
    ).length;
    res.json({
      status: "ok",

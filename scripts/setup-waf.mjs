...
async function deployManagedRuleset() {
  ...
  try {
    ...
    const managedRuleset = existing.find(
      (r) => r.phase === "http_request_firewall_managed" && r.name === "CyberAI Managed Rules",
    );
...
async function deployCustomRules() {
  ...
  const rules = [
    ...
    },
    {
      action: "block",
      expression:
        'http.request.uri.query contains "union select" or http.request.uri.query contains "1=1"',
    ...
    },
    {
      action: "block",
      expression:
        'http.request.uri.path contains ".env" or http.request.uri.path contains ".git" or http.request.uri.path contains ".htaccess"',
...
async function deployRateLimiting() {
  ...
  const rules = [
    ...
    },
    {
      action: "challenge",
      expression:
        'http.request.uri.path starts_with "/api" and not http.request.uri.path starts_with "/api/auth"',
      action_parameters: {
        ratelimit: {
          characteristics: ["cf.colo.id", "ip.src"],
          period: 60,

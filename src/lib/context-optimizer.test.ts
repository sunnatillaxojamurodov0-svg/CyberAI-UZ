...
describe("Context Optimizer", () => {
  ...
  describe("optimizeContext", () => {
    it("should return all messages if within token limit", () => {
      const history = [createMessage("user", "Hello"), createMessage("assistant", "Hi there")];
    ...
    it("should truncate long messages to fit token limit", () => {
      ...
      const history = Array(100)
        .fill(null)
        .map((_, i) => createMessage(i % 2 === 0 ? "user" : "assistant", longMessage));
    ...
    it("should prioritize recent messages", () => {
      const history = Array(50)
        .fill(null)
        .map((_, i) => createMessage(i % 2 === 0 ? "user" : "assistant", `Message ${i}`));
  ...
  describe("getContextStats", () => {
    it("should return correct stats", () => {
      const history = [createMessage("user", "Hello"), createMessage("assistant", "Hi")];

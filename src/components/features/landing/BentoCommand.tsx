      ...
      <div className="mx-auto grid max-w-7xl grid-cols-12 gap-4">
        ...
        <motion.div {...reveal(0)} className="col-span-12 md:col-span-8">
          <GlassPanel hoverGlow className="h-full min-h-[440px] p-8">
            <div className="absolute right-4 top-4 font-mono text-[10px] tracking-widest text-primary/60">REF: 00-ALPHA-9</div>
            <div className="flex h-full flex-col">
              <div className="mb-6">
                <h3 className="font-display text-2xl font-bold tracking-tight">Global Threat Visualizer</h3>
                <p className="mt-1 text-sm text-muted-foreground">Real-time telemetry across deep-space relays.</p>
              </div>
        ...
        <motion.div {...reveal(1)} className="col-span-12 md:col-span-4">
          <GlassPanel className="flex h-full min-h-[440px] flex-col gap-5 p-6 hover:border-accent/30 transition-colors">
            <div className="flex items-center justify-between">
              <span className="font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-accent">AI Assistant</span>
        ...
        <motion.div {...reveal(2)} className="col-span-6 md:col-span-3">
          <GlassPanel className="p-6">
            <div className="mb-4 font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Uptime</div>
        ...
        <motion.div {...reveal(3)} className="col-span-6 md:col-span-3">
          <GlassPanel className="p-6">
            <div className="mb-4 font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Threats Blocked</div>
            <div className="font-display text-4xl font-bold tracking-tighter">
              14.2<span className="text-accent">M</span>
            </div>
            <div className="mt-4 flex items-end gap-1">
              {[0.2, 0.3, 0.45, 0.6, 0.8, 1].map((h, i) => (
                <div key={i} className="w-1 rounded-sm bg-accent" style={{ height: `${h * 28}px`, opacity: 0.25 + h * 0.7 }} />
              ))}
            </div>
          </GlassPanel>
        </motion.div>
        ...
        <motion.div {...reveal(4)} className="col-span-12 md:col-span-6">
          <GlassPanel className="flex items-center justify-between p-6">
            <div>
              <div className="mb-1 font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground">System Status</div>
              <div className="font-display text-xl font-bold">All sectors operational</div>
            </div>
            <div className="flex -space-x-3">
              {["S1", "S2", "S3"].map((s, i) => (
                <div key={s} className="grid size-10 place-items-center rounded-full border-2 border-background bg-surface-2 font-mono text-[10px]" style={{ zIndex: 10 - i }}>
                  {s}
                </div>
              ))}

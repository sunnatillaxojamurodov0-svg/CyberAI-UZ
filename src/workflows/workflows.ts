...
type WorkflowStep = {
  ...
  do<T>(name: string, config: { retries?: { limit: number; delay: string | number; backoff: string }; timeout?: string }, fn: () => Promise<T>): Promise<T>;
...
type Env = {
  cyberai_db: { prepare(sql: string): { bind(...args: unknown[]): { first<T>(): Promise<T | null>; run(): Promise<unknown> } } };
  CYBERAI_KV: { get(key: string): Promise<string | null>; put(key: string, value: string, opts?: { expirationTtl?: number }): Promise<void> };
  AI: unknown;
  MY_BUCKET: unknown;
...
class ChallengeGenerator {
  ...
  async run(event: WorkflowEvent<ChallengeParams>, step: WorkflowStep) {
    ...
    const challenge = await step.do('generate challenge', {
      ...
      const difficultyNames: Record<number, string> = { 1: 'Easy', 2: 'Medium', 3: 'Hard', 4: 'Expert', 5: 'Insane' };
    ...
    await step.do('save challenge to database', {
      retries: { limit: 3, delay: '2 seconds', backoff: 'linear' },
    }, async () => {
      await this.env.cyberai_db
        .prepare('INSERT INTO challenges (id, name, difficulty, category, scenario, objectives, flag, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
        .bind(challenge.id, challenge.name, challenge.difficulty, challenge.category, challenge.scenario, challenge.objectives, challenge.flag, challenge.created_at)
        .run();
    });

    await step.do('assign challenge to user', {
      retries: { limit: 2, delay: '1 second', backoff: 'constant' },
    }, async () => {
      await this.env.cyberai_db
        .prepare('INSERT INTO user_challenges (user_id, challenge_id, assigned_at, status) VALUES (?, ?, ?, ?)')
    ...
    await step.do('create notification', async () => {
      await this.env.cyberai_db
        .prepare('INSERT INTO notifications (user_id, type, title, body, created_at) VALUES (?, ?, ?, ?, ?)')
        .bind(userId, 'challenge', 'New Challenge Assigned', `You have been assigned: ${challenge.name} (${challenge.difficultyName})`, Math.floor(Date.now() / 1000))
        .run();
    });

    await step.do('cache challenge in KV', async () => {
      await this.env.CYBERAI_KV.put(`challenge:${challenge.id}`, JSON.stringify(challenge), { expirationTtl: 86400 });
    });
...
class UserOnboarding {
  ...
  async run(event: WorkflowEvent<OnboardingParams>, step: WorkflowStep) {
    ...
    await step.do('verify user exists', {
    ...
    await step.do('create welcome notification', async () => {
      await this.env.cyberai_db
        .prepare('INSERT INTO notifications (user_id, type, title, body, created_at) VALUES (?, ?, ?, ?, ?)')
        .bind(userId, 'info', 'Welcome to CyberAI!', `Welcome ${name || email}! Start exploring CTF challenges and sharpen your cybersecurity skills.`, Math.floor(Date.now() / 1000))
        .run();
    });

    await step.do('generate starter challenge', {
    ...
    await step.do('initialize user stats', async () => {
      await this.env.CYBERAI_KV.put(`stats:${userId}`, JSON.stringify({
...
class ConsoleAnalysis {
  ...
  async run(event: WorkflowEvent<ConsoleParams>, step: WorkflowStep) {
    ...
    await step.do('validate session', {
      retries: { limit: 2, delay: '1 second', backoff: 'constant' },
    }, async () => {
      const session = await this.env.cyberai_db
        ...
        .first();
      if (!session) throw new NonRetryableError(`Session ${sessionId} not found for user ${userId}`);
    });

    const analysis = await step.do('analyze command history', {
      ...
      for (const cmd of commands) {
        ...
        if (cmd.includes('nmap') || cmd.includes('masscan')) attackPatterns.push('reconnaissance');
        ...
        if (cmd.includes('metasploit') || cmd.includes('msfconsole')) attackPatterns.push('exploitation');
        if (cmd.includes('gobuster') || cmd.includes('dirb') || cmd.includes('feroxbuster')) attackPatterns.push('directory_enumeration');
      ...
      const methodology = {
        ...
        enumeration: commands.some((c: string) => c.includes('gobuster') || c.includes('dirb') || c.includes('nikto')),
        exploitation: commands.some((c: string) => c.includes('sqlmap') || c.includes('exploit') || c.includes('msf')),
        post_exploitation: commands.some((c: string) => c.includes('ssh') || c.includes('scp') || c.includes('upload')),
      };
    ...
    await step.do('save analysis to database', {
      retries: { limit: 2, delay: '2 seconds', backoff: 'linear' },
    }, async () => {
      await this.env.cyberai_db
    ...
    if (analysis.score >= 75) {
      await step.do('update leaderboard', {
        retries: { limit: 2, delay: '1 second', backoff: 'constant' },
      }, async () => {
        ...
        if (!existing) {
          await this.env.cyberai_db
            .prepare('INSERT INTO leaderboard (user_id, challenge_id, score, time_seconds, tools_used, solved_at) VALUES (?, ?, ?, ?, ?, ?)')
            .bind(userId, challengeId, analysis.score, 0, JSON.stringify(Object.keys(analysis.tool_usage)), Math.floor(Date.now() / 1000))
            .run();
        }
      });

      await step.do('update challenge status', {
        retries: { limit: 2, delay: '1 second', backoff: 'constant' },
      }, async () => {
        await this.env.cyberai_db
          .prepare('UPDATE user_challenges SET status = ?, completed_at = ? WHERE user_id = ? AND challenge_id = ?')

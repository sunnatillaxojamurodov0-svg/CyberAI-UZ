...
export function ChallengeAdmin() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.ok) {
        setMessage({ type: "success", text: editingId ? "Challenge updated!" : "Challenge created!" });
        setShowForm(false);
        setEditingId(null);
          ...
          >
            ...
            <div className="flex items-center gap-3 p-3 rounded-lg border border-border bg-background">
              ...
              <div className="flex items-center gap-2">
                {form.dynamic_flags ? <Zap size={14} className="text-accent" /> : <ZapOff size={14} className="text-muted-foreground" />}
                <div>
                  <div className="text-sm font-medium">Dynamic Flags</div>
                  <div className="text-[11px] text-muted-foreground">
                    {form.dynamic_flags ? "Each user gets a unique flag" : "Same flag for all users"}
                  </div>
                </div>
              </div>
            </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border">
                <th className="text-left py-2 font-mono text-[10px] font-bold uppercase tracking-[0.15em] text-muted-foreground">Name</th>
                <th className="text-left py-2 font-mono text-[10px] font-bold uppercase tracking-[0.15em] text-muted-foreground">Category</th>
                <th className="text-left py-2 font-mono text-[10px] font-bold uppercase tracking-[0.15em] text-muted-foreground">Difficulty</th>
                <th className="text-left py-2 font-mono text-[10px] font-bold uppercase tracking-[0.15em] text-muted-foreground">Flag</th>
                <th className="text-left py-2 font-mono text-[10px] font-bold uppercase tracking-[0.15em] text-muted-foreground">Dynamic</th>
                <th className="text-right py-2 font-mono text-[10px] font-bold uppercase tracking-[0.15em] text-muted-foreground">Actions</th>
              </tr>
            </thead>
            <tbody>
              {challenges.map((c) => {
                ...
                return (
                  <tr key={c.id} className="border-b border-border/50 hover:bg-muted/30">
                    ...
                    <td className="py-3">
                      <span className="rounded bg-muted px-2 py-0.5 text-[10px] font-mono uppercase">{c.category}</span>
                    </td>
                    <td className="py-3">
                      <span className={`font-mono text-xs ${diff?.color}`}>{diff?.label}</span>
                    </td>
                    <td className="py-3 font-mono text-xs text-muted-foreground truncate max-w-[200px]">{c.flag}</td>
                    <td className="py-3">
                      {c.dynamic_flags ? (
                        <span className="flex items-center gap-1 text-accent text-xs"><Zap size={12} /> On</span>
                      ) : (
                        <span className="text-muted-foreground text-xs">Off</span>
                      )}
                    </td>
                    <td className="py-3 text-right">
                      <button onClick={() => handleEdit(c)} className="p-1 text-muted-foreground hover:text-foreground"><Edit size={14} /></button>
              ...
              {challenges.length === 0 && (
                <tr><td colSpan={5} className="py-8 text-center text-muted-foreground text-sm">No challenges yet</td></tr>
              )}
            </tbody>
          </table>
        </div>
